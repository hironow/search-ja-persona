...
3. Restore the full corpus only when needed (see Context) — day-to-day the repo runs on the QA sample, generated with `just qa-sample` (not checked in).
...
- The 1k QA sample (`qa_samples/qa_sample.parquet`) is **git-ignored and not checked in** — it is generated by `just qa-sample` (which needs the dataset above), so a fresh clone must generate it before `just qa` or the marimo catalog.
...
- `just qa` — index + search the 1k sample (`just qa-sample` generates it first; quick smoke check)
- `just notebook` — open the marimo feature-catalog notebook (`marimo/catalog.py`)
